        ...
        if targets is not None:
            ...
            if P1_x.is_cuda:
                t1_x, t1_y, t2_x, t2_y, t3_x, t3_y, t4_x, t4_y, mask, tcls = \
                    t1_x.cuda(), t1_y.cuda(), t2_x.cuda(), t2_y.cuda(), t3_x.cuda(
                    ), t3_y.cuda(), t4_x.cuda(), t4_y.cuda(), mask.cuda(), tcls.cuda()
            ...
            if nM > 0:
                ...
                lcls = (k / self.nC) * \
                    CrossEntropyLoss(pred_cls[mask], torch.argmax(tcls, 1))
            else:
                lx1, ly1, lx2, ly2, lx3, ly3, lx4, ly4, lcls, lconf = \
                    ...
                    FT([0]).requires_grad_(True), FT([0]).requires_grad_(True), FT([0]).requires_grad_(
                        True), FT([0]).requires_grad_(True), FT([0]).requires_grad_(True),
            ...
            if i.sum() > 0:
                ...
                FPe = torch.bincount(
                    FP_classes, minlength=self.nC).float().cpu()
            else:
